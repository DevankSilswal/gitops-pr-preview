...
#
# Opting in is a topic on the repository plus a .preview.yml in its root.
# Nobody has to be asked, and nobody has to approve — which is the point, and
# also why the two decisions below are made the way they are.
...
results.each do |repo|
  ...
  begin
    raw = gh('api', "repos/#{full}/contents/.preview.yml", '--jq', '.content')
    config = YAML.safe_load(raw.strip.unpack1('m'))
  rescue StandardError
    skipped << [full, 'no .preview.yml in the repository root']
    next
  end

  unless config.is_a?(Hash)
    skipped << [full, '.preview.yml is not a mapping']
    next
  end
...
onboarded.each do |entry|
  ...
  body = +"# Generated by scripts/discover-repos.rb — do not edit.\n" \
          ...
          "# and a .preview.yml. Removing either removes this file, and with it\n" \
